...
from sklearn.cluster import (
    AffinityPropagation, # utilise des similarités entre données
    AgglomerativeClustering, # fusionne progressivement les clusters proches
    Birch, # utilise des sous-clusters et un modèle hiérarchique
    DBSCAN, # identifie des clusters de densité variable
    KMeans,  # divise en k clusters en minimisant les distances
    MiniBatchKMeans,  # est une version par lots de KMeans
    MeanShift, # se déplace vers les zones de densité maximale
    OPTICS, # organise les clusters basés sur la densité et la connectivité
    SpectralClustering, # utilise la structure des spectres pour partitionner les données
)
from sklearn.exceptions import ConvergenceWarning
from sklearn.metrics import classification_report
from sklearn.mixture import GaussianMixture   # suppose à partir d'un mélange de distributions gaussiennes
...
# Fonction pour créer une table de log pour comparer les performances des modèles
...
# Fonction pour tester un modèle de clustering non supervisé spécifique
def test_model(
    query_select,
    query_where,
    num_samples=0,
...
# Fonction pour afficher un graphique des données réelles à des fins comparatives
...
# Fonction pour exécuter les tests sur une liste de modèles et afficher le meilleur résultat
...
# Fonction spécifique pour exécuter un test sur des données CO2e, EC, et MRO
...
# Liste des modèles de clustering à tester avec leurs paramètres
test_model_list = [
    {
...
# Exécution des tests et affichage du meilleur modèle pour les données CO2e, EC, et MRO
label_encoders = {}
